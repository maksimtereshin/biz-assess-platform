...
export class PaymentController {
  ...
  async createInvoice(@Param('sessionId') sessionId: string, @Body() body: { userId: number }) {
  ...
  private async handleSuccessfulPayment(payment: any): Promise<any> {
    ...
    // Verify payment
    const success = await this.paymentService.verifyPayment(payment.telegram_payment_charge_id, paymentId);
    
    return {
      ok: success,
      message: success ? 'Payment processed successfully' : 'Payment verification failed',
    };
  }
}
