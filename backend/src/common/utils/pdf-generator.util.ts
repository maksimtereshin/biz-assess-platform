...
export class PdfGenerator {
   ...
   */
  async createPdf(data: AnalyticsResult, isPaid: boolean = false): Promise<Buffer> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // Add overall score
        doc.fontSize(18)
           .text(`Overall Business Score: ${data.overallScore}%`, { align: 'center' })
           .moveDown(1);
        ...
        doc.fontSize(12)
           ...
           .text(`Completion Rate: ${Math.round((data.answeredQuestions / data.totalQuestions) * 100)}%`)
           .moveDown(2);

        // Add category scores with pie chart
        doc.text('Category Performance:', { underline: true })
           .moveDown(0.5);
        ...
        if (isPaid || this.shouldShowSubcategoryDetails(data)) {
          doc.text('Detailed Subcategory Analysis:', { underline: true })
             .moveDown(0.5);

          Object.entries(data.subcategoryScores).forEach(([subcategory, score]) => {
            const subcategoryName = this.formatSubcategoryName(subcategory);
            doc.text(`${subcategoryName}: ${score}%`)
               .moveDown(0.3);
          });
        ...
        // Add footer
        doc.fontSize(10)
           .text('Generated by Business Assessment Platform', { align: 'center' })
           .text(new Date().toLocaleDateString(), { align: 'center' });

        if (!isPaid) {
          doc.moveDown(1)
             .text('Upgrade to Full Report for detailed insights and action plans', { align: 'center' });
        }
  ...
  private shouldShowSubcategoryDetails(data: AnalyticsResult): boolean {
    ...
    return Object.keys(data.subcategoryScores).some(key => 
      key.includes('delegation') || key.includes('economics')
    );
  }
  ...
  private generateRecommendations(data: AnalyticsResult): string[] {
    ...
    Object.entries(data.categoryScores).forEach(([category, score]) => {
      if (typeof score === 'number' && score < 50) {
        switch (category) {
          case 'product':
            recommendations.push('Focus on defining your unique value proposition and product-market fit');
            break;
          case 'marketing':
            recommendations.push('Develop a comprehensive marketing strategy and improve customer targeting');
            break;
          case 'sales':
            recommendations.push('Optimize your sales process and improve conversion rates');
            break;
          case 'hr':
            recommendations.push('Invest in team development and improve delegation processes');
            break;
          case 'automation':
            recommendations.push('Implement automation tools to improve efficiency');
            break;
          case 'metrics':
            recommendations.push('Establish key performance indicators and regular reporting');
            break;
        }
      }
    });
    ...
    if (recommendations.length === 0) {
      recommendations.push('Continue monitoring and optimizing your business processes');
      recommendations.push('Consider expanding into new markets or product lines');
    }
   ...
   */
  private async generatePieChart(categoryScores: Record<string, number>): Promise<Buffer | null> {
    try {
      const canvas = createCanvas(400, 300);
      const ctx = canvas.getContext('2d');

      // Prepare data for the chart
      const labels = Object.keys(categoryScores).map(key => this.formatCategoryName(key));
...
${Object.entries(data.categoryScores)
  .map(([category, score]) => `- ${this.formatCategoryName(category)}: ${score}%`)
  .join('\n')}

Subcategory Scores:
${Object.entries(data.subcategoryScores)
  .map(([subcategory, score]) => `- ${this.formatSubcategoryName(subcategory)}: ${score}%`)
  .join('\n')}
    `.trim();
  }
}
