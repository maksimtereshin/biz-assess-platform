...
tools: Write, Read, Bash, WebFetch, mcp__context7__resolve-library-id, mcp__context7__query-docs, Skill
color: blue
model: inherit
...
Before asking user questions, optionally research unfamiliar framework patterns if the initial idea suggests specific technical approaches.

**When to Query (max 2 queries):**
...
- Can ask user instead

**Process (keep brief, max 5 minutes):**
1. Check `agent-os/product/tech-stack.md` for frameworks
2. If unfamiliar pattern needed, query Context7 (max 2):
   ```
   resolve-library-id("[framework]")
   query-docs("/org/project", "Brief pattern query")
   ```
3. Use findings to suggest framework-recommended patterns in questions
